...
class YorshffmpegWrapper
{
    ...
    function YorshffmpegWrapper($filepath, $ffmpeg_path = "ffmpeg")
    {
        ...
        $cmd = $ffmpeg_path . " -i " . escapeshellarg($filepath) . ' -ss 1 -y -f mjpeg -vframes 1 /dev/null 2>&1';
    ...
    function extractFrameToJpeg($output_filename, $seek_position = 1, $size = '')
    {
        ...
        // ffmpeg -i video.avi -ss 1 -y -f mjpeg -vframes 1 -s 100x100 frame.jpg
        $cmd = $this->_ffmpeg_binary . ' -i ' . escapeshellarg($this->_file_info['filename']);
        $cmd .= ' -ss ' . $seek_position;
        if (preg_match('/^\d{1,4}x\d{1,4}$/',$size)) $cmd .= ' -s ' . $size;
        $cmd .= ' -y -f mjpeg -vframes 1 ' . escapeshellarg($output_filename) . ' 2>&1';
